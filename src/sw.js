...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches
      .keys()
      .then((keys) => Promise.all(keys.filter((k) => k.startsWith('billsplit-') && k !== CACHE).map((k) => caches.delete(k))))
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    data = { title: '算錢用ㄉ東西', body: event.data ? event.data.text() : '' }
  }
  const title = data.title || '算錢用ㄉ東西'
  const options = {
    body: data.body || '',
